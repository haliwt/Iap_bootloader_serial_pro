...
void stmflash_write_nocheck(uint32_t waddr, uint16_t *pbuf, uint16_t length)
{
    ...
    for (i = 0; i < length; i++)
    {
        //HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, waddr, pbuf[i]);//FLASH_TYPEPROGRAM_FAST
       HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, waddr, pbuf[i]); //
        waddr += 2; /* 指向下一个半字 */
    }
}
...
void stmflash_write(uint32_t waddr, uint16_t *pbuf, uint16_t length)
{
    ...
    while (1)
    {
        ...
        { 
            ...
            flash_eraseop.Page = secpos * STM32_SECTOR_SIZE + STM32_FLASH_BASE;  /* 要擦除的扇区 */
